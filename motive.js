(function(){
	...
	var transpose = function(pitch_name, direction, interval){
		...
		if (direction !== 'up' && direction !== 'down') {
			throw 'ERROR: transposition direction invalid or not specified';
			return pitch_name;
		}
		...
		if(input_operator) {
			for(var i = 0; i < operators.length; i++){
				if (input_operator === operators[i].name){
					alter = operators[i].value;
					break;
				}
			}
		}
		...
		for (var i = 0; i < steps.length; i++) {
			if (step === steps[i].name) {
				step_index = i;
				input_value = (steps[i].value + alter) % 12;
				break;
			}
		}
		...
		if (target_value === (12 + steps[target_step_index].value) % 12) {
			...
			for (var i = 0; i < operators.length; i++) {
				if (target_value === (12 + steps[target_step_index].value + operators[i].value) % 12) {
					return steps[target_step_index].name + operators[i].name;
				}
			}
		}
	}; //end transpose
	...
	var getRandomInterval = function(simple, normalized) {
		...
		var interval;
		if (simple) {
			var rnd = Math.floor(Math.random() * simple_intervals.length);
			interval = simple_intervals[rnd]; 
		} else {
			var rnd = Math.floor(Math.random() * intervals.length);
	...
	Motive.Note = function(name) {
		if (!name) {
			throw 'ERROR: You must specify a note name or midi number';
			return;
		}
		switch (typeof name){
			case 'string': {
				try {
					...
					if (!step) {
						throw 'ERROR: Note name must begin with a capital letter [A-G].';
						return;
					}
					...
					if (octave){
						...
						for (var i = 0; i < steps.length; i++){
							if (steps[i].name === step){
								midi = (12 * octave) + 12 + (steps[i].value % 12) + alter;
							}
						}
						if (midi) {
							...
							throw 'ERROR: could not calculate midi note number, check octave value';
							return;
						}
	...
	Motive.Chord = function(name){
		...
		var note_members = [];
		for (var i = 0; i < note_member_names.length; i++) {
			note_members.push(new Motive.Note(note_member_names[i]));
		}
	...
	Motive.Scale = function(name){
		if (!name) {
			throw 'ERROR: no scale name specified.';
			return;
		}
		if (typeof name === 'string') {
			...
			if (!name.match(scale_pattern)) {
				throw 'ERROR: scale name was not formatted correctly';
				return;
			}
			...
			if(matched){
				...
				throw 'ERROR: scale name did not match any known scales.';
				return;
			}
			...
			throw 'ERROR: scale name must be a string';
			return;
		}
	};
